	...
	public static void CheckForGesture( uint userId, ref GestureData gestureData, float timestamp, ref Vector3[] jointsPos, ref bool[] jointsTracked )
	{
		...
		switch ( gestureData.gesture )
		{
			...
			case Gestures.NoNo:
				...
				{
					case 0:
						if ( jointsTracked[ rightHandIndex ] && jointsTracked[ rightShoulderIndex ] && jointsTracked[ headIndex ] && jointsPos[ rightHandIndex ].z - jointsPos[ rightShoulderIndex ].z < -0.1f )
						{
							//	Debug.Log( "Right hand is infront of right shoulder" );
					...
					case 1:
						bool isInPose = jointsTracked[ rightHandIndex ] && jointsTracked[ rightShoulderIndex ] && jointsTracked[ headIndex ]
							&& jointsPos[ rightHandIndex ].z - jointsPos[ rightShoulderIndex ].z < 0.1f && jointsPos[headIndex].x - gestureData.jointPos.x < -0.1f;
					...
					case 2:
						...
						isInPose = jointsTracked[ rightHandIndex ] && jointsTracked[ rightShoulderIndex ] && jointsTracked[ headIndex ]
							&& jointsPos[ rightHandIndex ].z - jointsPos[ rightShoulderIndex ].z < 0.1f && jointsPos[ headIndex ].x - gestureData.lastPos.x > 0.1f;
						...
						if ( isInPose )
						{
							...
							CheckPoseComplete( ref gestureData, timestamp, jointsPos[ headIndex ], isInPose, 0f );
						}
