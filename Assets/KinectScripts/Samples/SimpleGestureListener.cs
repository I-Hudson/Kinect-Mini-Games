...
public class SimpleGestureListener : MonoBehaviour, KinectGestures.GestureListenerInterface
{
	...
	public void UserDetected( uint userId, int userIndex )
	{
		...
		manager.DeleteGesture( userId, KinectGestures.Gestures.NoNo );
								  ...
								  KinectWrapper.NuiSkeletonPositionIndex joint, Vector3 screenPos )
	{
		...
		//
		if ( gesture == KinectGestures.Gestures.HandsUp )
		{
			GameObject.FindObjectOfType<GameController>().HandsUp();
		}

		if(gesture == KinectGestures.Gestures.NoNo)
		{
		...
		{
			if ( GameObject.FindObjectOfType<ShootController>() )
			{
				GameObject.FindObjectOfType<ShootController>().Shoot();
			}
		}
